...
class Solution:
    ...
    def minCost(self, arr, n):
        ...
        # code here
        for i in range(0, n):
            heappush(min_cost, arr[i])

        size = n
        total_min_cost = 0
        while size > 1:
            rope_1_cost = heappop(min_cost)
            size -= 1
            rope_2_cost = heappop(min_cost)
            size -= 1

            cost = rope_1_cost + rope_2_cost
            heappush(min_cost, cost)
            size += 1
            total_min_cost += cost
...
class TestSolution(unittest.TestCase):
    ...
    def test_case_1(self):
        self.assertEqual(self.obj.minCost([4,2,7,6,9],5),62)

    def test_case2(self):
        self.assertEqual(self.obj.minCost([4,3,2,6],4),29)